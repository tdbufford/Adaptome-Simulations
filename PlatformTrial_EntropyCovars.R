...
### Normal outcomes                                                       ####
### with Entropy Balancing + Covariates in the Bayesian model             ####
###############################################################################
...
trialSimEntropyC = function(txEffects, sigma2, arms, NperInterim, maxSample, 
  ...
  while(totalSample <= maxSample && modelConverge == TRUE && 
    ...
    #add covariates in addition to treatment group indicators
    x_model_extra = dataset[, 1:6]
    ...
    model_filename = paste("JAGSmodel_EBC_", trialName, "_Outcome", outcomeDesign, 
                           ".txt", sep="")
    sink(model_filename)
    cat("
          model{
            for(i in 1:N){
              ...
              mu[i] <- inprod(x1[i,], theta[]) + inprod(x2[i,], zeta[])
            }
            for(j in 1:arms){
    ...
    chains = 3
    inits1 = rep(list(list(theta = rep(0.5, arms), 
                           prec = 1,
                           zeta = rep(0.5, 6))), chains)
    iter = 11000
    burnin = 1000
    thin = 2
    # parameters to track
    ...
    summary = trialSummaryNormal(model, txNames, arms, trialNum, txNums, k, 
                                 ...
                                 futThresh)
    summary[, 9] = as.numeric(summary[, 9])
