...
### Normal outcomes                                                       ####
### with NO Entropy Balancing & NO adjustment for covariates              ####
###############################################################################
...
#                   accomodate more than 4. To increase, edit code at line 415
...
#              versions
# @param outcomeDesign - 1 = Linear, 2 = Non-linear
#              
...
trialSimNoEntropy = function(txEffects, sigma2, arms, NperInterim, maxSample, 
  ...
  thetapriors = cbind(rep(0, arms), rep(10, arms))
  ...
  covar = as.matrix(rbind(c(2,     1,   -1),
                          c(1,     1, -0.5),
                          c(-1, -0.5,    1)))
  x123_means = c(0, 0, 0)
  
  x1x2x3 = mvrnorm(1000, mu = x123_means, Sigma = covar)
  x4 = runif(1000, min = -3, max = 3)
  x5 = rchisq(1000, df = 1, ncp = 0)
  x6 = rbinom(1000, size = 1, prob = 0.5)
  
  external_sample = cbind(x1x2x3, x4, x5, x6)
  colnames(external_sample) = c("x1", "x2", "x3", "x4", "x5", "x6")
  ...
  # continue simulation until max sample size is reached or model fails to 
  # converge or max iteration is reached
  while(totalSample <= maxSample && modelConverge == TRUE && 
        maxIteration <= 4 && arms > 1){
    ...
    ## 3.2) Generate Data
    #We use 6 covariates Xj 
    #X1, X2, and X3 ~ multivariate normal with means zero, 
    #variances of (2, 1, 1) and covariances of (1,-1,-0.5) respectively
    #X4 ~ uniform on [-3, 3]; 
    #X5 ~ chisquared (1)
    #X6 ~ Bernoulli with mean 0.5
    
    datasize = length(txEffects) * NperInterim * 2
    ...
    x1x2x3 = mvrnorm(datasize, mu = x123_means, Sigma = covar)
    x4 = runif(datasize, min = -3, max = 3)
    x5 = rchisq(datasize, df = 1, ncp = 0)
    x6 = rbinom(datasize, size = 1, prob = 0.5)
    
    epsilon1 = rnorm(datasize, mean = 0, sd = 1)
    epsilon2 = rnorm(datasize, mean = 0, sd = 1)
    epsilon3 = rnorm(datasize, mean = 0, sd = 1)
    epsilon4 = rnorm(datasize, mean = 0, sd = 1)
    epsilon5 = rnorm(datasize, mean = 0, sd = 1)
    epsilon6 = rnorm(datasize, mean = 0, sd = 1)
    epsilon7 = rnorm(datasize, mean = 0, sd = 1)
    epsilon8 = rnorm(datasize, mean = 0, sd = 1)

    X = cbind(x1x2x3, x4, x5, x6, epsilon1, epsilon2, epsilon3, epsilon4, 
    ...
    # Linear models for assignment of observations to groups
    beta2 = as.matrix(c(   0,  0.7,  0.5, -0.1, -0.2, -0.1,  0.5, 0, 0, 0, 0, 0, 0, 0))
    beta3 = as.matrix(c(   1, -0.5,  1.5, -0.2, -0.2,   -1,  0, 0.5, 0, 0, 0, 0, 0, 0))
    beta4 = as.matrix(c( 0.1,  0.3,    0,  0.1, -0.3, 0.25,  0, 0, 0.5, 0, 0, 0, 0, 0))
    beta5 = as.matrix(c(-0.2,    0,  0.3,    0,  0.2,   -1,  0, 0, 0, 0.5, 0, 0, 0, 0))
    beta6 = as.matrix(c(0.75,   -1, -0.5, -0.1,    0, -1.5,  0, 0, 0, 0, 0.5, 0, 0, 0))
    beta7 = as.matrix(c( 0.5,    1,    1,  0.1, -0.1, -1.5,  0, 0, 0, 0, 0, 0.5, 0, 0))
    beta8 = as.matrix(c(-0.2,  0.1, -0.8,  0.2, -0.1, -0.8,  0, 0, 0, 0, 0, 0, 0.5, 0))
    beta9 = as.matrix(c( 0.4, -0.2,  0.3,  0.1,  0.1, -0.4,  0, 0, 0, 0, 0, 0, 0, 0.5))
    ...
    denominator = 1 + exp(score2) + exp(score3) + exp(score4) + exp(score5) + 
    ...
    # organize vectors of indicators with treatment assignments
    ...
    eta = rnorm(datasize, mean = 0, sd = 1)
    
    
    if(outcomeDesign == 1){
      #Outcome Design 1: Y = X1 + X2 + X3 - X4 + X5 + X6 + eta
      Outcome = data.frame(outcome = (X[, 1] + X[, 2] + X[, 3] - X[, 4] + X[, 5] + X[, 6] + eta))
    }else if (outcomeDesign == 2){
      #Outcome Design 2: Y = X1 + X2 + 0.2*X3*X4 - sqrt(X5) + eta
      Outcome = data.frame(outcome = (X[, 1] + X[, 2] + 0.2*X[, 3]*X[, 4] - sqrt(X[, 5]) + eta))
    }
    ...
    # dataset %>%
    #   ggplot() +
    #   geom_density(aes(x = x5, group = treatment, color = as.factor(treatment))) +
    #   theme_bw()
    # 
    # dataset %>%
    #   ggplot() +
    #   geom_density(aes(x = x6, group = treatment, color = as.factor(treatment))) +
    #   theme_bw()
    ##############################################################################
    ...
    X_squares = getsquares(dataset[,11:16])
    X_squares = data.frame(X_squares)
    
    cbind(X_squares, dataset[,-(11:16)]) -> dataset
    ...
    for(i in 1:arms){
      assign(paste("eb.out", i, sep=""),
             ebalance(Treatment = c(get(paste("txgroup", i, sep=""))$treatment, 
                                   rep(1, times = 1000)),
                      X = rbind((get(paste("txgroup", i, sep="")))[,1:6], 
                                external_sample[,1:6]),
                      constraint.tolerance = 2#, norm.constant = 400
                      )
      )
      assign(paste("eb.out", i, sep=""),
    ...
    model_filename = paste("JAGSmodel_", trialName, "_Outcome", outcomeDesign, 
                           ".txt", sep="")
    sink(model_filename)
    cat("
          model{
    ...
    summary = trialSummaryNormal(model, txNames, arms, trialNum, txNums, k, 
                                 ...
                                 futThresh)
    if(outcomeDesign == 1){
      # 1.5 is baseline mean outcome from adding covariates for linear design
      summary[, 9] = as.numeric(summary[, 9]) + 1.5 
    }else if(outcomeDesign == 2){
      # -0.8 is baseline mean outcome from adding covariates for nonlinear design
      summary[, 9] = as.numeric(summary[, 9]) - 0.8
    }
    ...
    write.table(summary, 
                file = paste("platformTrialSummary_", trialName, "_Outcome", 
                             outcomeDesign, ".csv", sep = ""), 
                row.names = FALSE, col.names = (k == 1 && trialNum == 1), na = "", 
                sep = ",", append = (k > 1 | trialNum > 1))
    
    # save variance summary
    write.table(varSummary, 
                file = paste("platformTrialVaraince_", trialName, "_Outcome", 
                             outcomeDesign, ".csv", sep = ""), 
                row.names = FALSE, col.names = (k == 1 && trialNum == 1), na = "", 
                sep = ",", append = (k > 1 | trialNum > 1))
  ...
  write.table(totalSample, sep = ",", append = (trialNum > 1), 
              ...
              file = paste("totalSampleSizes_", trialName, "_Outcome", 
                           outcomeDesign, ".csv", sep = ""))
  ...
  if(str_detect(trialName, "Random")){
    write.table(t(txEffects), sep = ",", append = (trialNum > 1), 
                ...
                file = paste("txEffectOrder_", trialName, "_Outcome", 
                             outcomeDesign, ".csv", sep = ""))
  }
}
